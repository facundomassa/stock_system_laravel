...
<nav class="navbar navbar-dark bg-dark sticky-top shadow-sm">
    <div class="container-fluid px-3">
        ...
        <!-- Toggler + Brand -->
        <button class="navbar-toggler border-0 me-2 d-block" type="button" data-bs-toggle="offcanvas"
        ...
        <div class="ms-auto d-flex align-items-center gap-3">
            ...
            @if(Auth::check() && Session::has('selected_operation'))
                <div class="dropdown">
                    <button class="btn btn-sm btn-outline-light dropdown-toggle px-3 py-1" type="button"
                        ...
                        @foreach(get_allowed_operations() as $operation)
                            <li>
                                <form action="{{ url('operation-select') }}" method="POST">
                                    @csrf
                                    <button type="submit" class="dropdown-item" name="selected_operation"
            ...
            @if (Auth::check())
                <div class="dropdown">
                    <a class="text-light text-decoration-none d-flex align-items-center gap-2 dropdown-toggle" href="#"
...
<div class="offcanvas offcanvas-start bg-dark text-bg-dark" tabindex="-1" id="sidebarMenu"
    ...
    <div class="offcanvas-body p-0">
        <ul class="navbar-nav flex-column fs-5">
            ...
            @if(auth()->check() && auth()->user()->hasRole('tecnico'))
                <li class="nav-item">
                    <a class="nav-link px-4 py-3 {{ request()->is('technical/dashboard') ? 'active' : '' }}"
                        href="{{ route('technical.dashboard') }}">
                        <i class="bx bx-tachometer me-2"></i> Dashboard Técnico
                    </a>
                </li>
            @else
                ...
                <li class="nav-item">
                    <a class="nav-link px-4 py-3 {{ request()->is('home') ? 'active' : '' }}" href="{{ url('/home') }}">
                        <i class="bx bx-tachometer me-2"></i> Dashboard Principal
                    </a>
                </li>
            @endif
            <!-- Ocultar todo lo demás para Técnicos -->
            @if(!auth()->check() || !auth()->user()->hasRole('tecnico'))
                ...
                <li class="nav-item">
                    <a class="nav-link px-4 py-3 {{ request()->is('direction') ? 'active' : '' }}"
                        href="{{ url('/direction') }}">
                ...
                <li class="nav-item">
                    <a class="nav-link px-4 py-3 {{ request()->is('movement') ? 'active' : '' }}"
                        href="{{ url('/movement') }}">
                        <i class="bx bx-transfer me-2"></i> Movimientos
                    </a>
                </li>
                <li class="nav-item">
                    <a class="nav-link px-4 py-3 {{ request()->is('transit') ? 'active' : '' }}"
                        href="{{ url('/transit') }}">
                ...
                <li class="nav-item">
                    <a class="nav-link px-4 py-3 {{ request()->is('article') ? 'active' : '' }}"
                        href="{{ url('/article') }}">
                        <i class="bx bx-news me-2"></i> Artículos
                    </a>
                </li>
                <li class="nav-item">
                    <a class="nav-link px-4 py-3 {{ request()->is('stockcenter') ? 'active' : '' }}"
                        href="{{ url('/stockcenter') }}">
                        <i class="bx bx-buildings me-2"></i> Centros de Stock
                    </a>
                </li>
                <li class="nav-item">
                    <a class="nav-link px-4 py-3 {{ request()->is('stock') ? 'active' : '' }}" href="{{ url('/stock') }}">
                        <i class="bx bx-box me-2"></i> Stock General
                    </a>
                </li>
                ...
                @if (get_selected_operation() === 'ADMIN')
                    <li class="nav-item">
                        <a class="nav-link px-4 py-3 {{ request()->is('admin/users') ? 'active' : '' }}"
                            href="{{ url('/admin/users') }}">
                            <i class="bx bx-shield me-2"></i> Usuarios
                        </a>
                    </li>
                    <li class="nav-item">
                        <a class="nav-link px-4 py-3 {{ request()->is('operation') ? 'active' : '' }}"
                            href="{{ url('/operation') }}">
                            <i class="bx bx-cog me-2"></i> Operaciones
                        </a>
                    </li>
                    <li class="nav-item">
                        <a class="nav-link px-4 py-3 {{ request()->is('importar') ? 'active' : '' }}"
